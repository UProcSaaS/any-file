...
function AnyFile(opt) {
	...
	var __processFileCompressOrUncompressZip = function(from, to, compress, callback) {
		...
		if (compress) {
			...
			yauzl.open(oldFile, {lazyEntries: true}, function(err, zipfile) {
				if (!err) {
					zipfile.readEntry();
	...
	var __toS3 = function(callback) {
		try {
			...
			cfg.client.upload({Body: file}).
			  send(function(err, data) {return callback(null, true);});
		} catch (e) {
			debug(e);
			if (callback) {
				return callback(e, false);
	...
	var __fromHttp = function(callback) {
	  ...
	  request(fromFile).pipe(file).on('error', function(err) {
			if (callback) {
				return callback(err, false);
			}
	  });
	...
	var __initFtp = function(file, callback) {
		...
		__isOpenPort(parsed.host, parsed.port, function(err, res) {
			if (err) {
				...
				return callback(null, {config: config, remoteFile: parsed.remoteFile.replace("/", "")});
			}
		});
	};
	...
	var __move = function(from, to, callback) {
		...
		source.pipe(dest);
		source.on('end', function() { return callback(null, true); });
		source.on('error', function(err) { return callback(err, false); });
	};
